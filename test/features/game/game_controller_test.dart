...
void main() {
  ...
  test('selecting a piece exposes its legal targets', () {
    ...
    expect(state().targets, containsAll([Square.parse('e3'), Square.parse('e4')]));
  });
  ...
  test('tapping another own piece reselects it', () {
    ...
    expect(state().targets, containsAll([Square.parse('d3'), Square.parse('d4')]));
  });
  ...
  test('promotion defaults to a queen', () {
    ...
    expect(state().position.pieceAt(Square.parse('a8')),
  ...
  test('exposes check and the checked king square', () {
    ...
    controller().loadFen('rnbqkbnr/ppppQppp/8/8/8/8/PPPP1PPP/RNB1KBNR b KQkq - 0 1');
    expect(state().isInCheck, isTrue);
    expect(state().checkSquare, Square.parse('e8'));
  });

  test('reports a checkmate outcome with the winner', () {
    controller().loadFen('rnb1kbnr/pppp1ppp/8/4p3/6Pq/5P2/PPPPP2P/RNBQKBNR w KQkq - 1 3');
